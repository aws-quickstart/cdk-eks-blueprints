 ...
 * controlled by ACL.
 */
export const DEFAULT_BUILD_POLICIES = [
  new PolicyStatement({
    resources: ['*'],
    actions: [
      'sts:AssumeRole',
  ...
  public wave(...waves: PipelineWave[]): CodePipelineBuilder {
    waves.forEach((wave) => {
      ...
      wave.stages.forEach((stage) =>
        this.props.stages?.push({ ...stage, ...{ waveId: wave.id } })
      );
    });
    return this;
  }
  ...
  constructor(
    ...
    if (pipelineProps.crossAccountKeys) {
      super(
        scope,
        id,
        withUsageTracking(CodePipelineStack.USAGE_ID_MULTI_ACCOUNT, props)
      );
    } else {
      ...
      for (let i in stages) {
        ...
        if (stage.waveId) {
          if (currentWave == null || currentWave.id != stage.waveId) {
            ...
            currentWave = pipeline.addWave(stage.waveId, {
              ...waveProps.props,
            });
          }
...
export class ApplicationStage extends cdk.Stage {
  ...
  constructor(
    scope: cdk.Stack,
    id: string,
    builder: StackBuilder | AsyncStackBuilder,
    props?: cdk.StageProps
  ) {
    super(scope, id, props);
    if ((<AsyncStackBuilder>builder).buildAsync !== undefined) {
      this.asyncTask = (<AsyncStackBuilder>builder).buildAsync(
        this,
        `${id}-blueprint`,
        props
      );
