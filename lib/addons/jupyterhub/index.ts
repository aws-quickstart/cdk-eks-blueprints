...
export class JupyterHubAddOn extends HelmAddOn {
    ...
    deploy(clusterInfo: ClusterInfo): Promise<Construct> {
        ...
        if (enableIngress){
            const dep = clusterInfo.getScheduledAddOn(AwsLoadBalancerControllerAddOn.name);
            assert(dep, `Missing a dependency: ${AwsLoadBalancerControllerAddOn.name}. Please add it to your list of addons.`); 
