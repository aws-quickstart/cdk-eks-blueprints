...
import { BlueprintBuilder, enabledControlPlaneLogTypes, stacks } from '../lib';
...
function getConstraintsDataSet(): BlueprintBuilder[] {
  ...
  const blueprint2 = blueprints.EksBlueprint.builder().addOns(...addOns).enableControlPlaneLogTypes(enabledControlPlaneLogTypes.api);
...
var longName: string = 'eks-blueprint-with-an-extra-kind-of-very-long-name-that-is-definitely-not-going-to-work';

test("Testing ID length error!", () => {
  expect(() => getConstraintsDataSet().at(1)?.build(app, longName))
    .toThrow!('Managed Node Groups ID must be no more than 63 characters long!');
});

test("Testing name length error!", () => {
  expect(() => getConstraintsDataSet().at(0)?.build(app, "longName"))
    .toThrow('Managed Node Groups name must be no more than 63 characters long!');
});
