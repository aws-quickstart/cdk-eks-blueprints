...
In the [Getting Started]() guide, we bootstrapped an EKS cluster with the workloads contained in teh ssp-workloads repository. Below, we will demonstrate how we can apply network policies to govern traffic between the workloads once Calico is installed.
