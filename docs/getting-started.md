...
npm install -g aws-cdk@2.1129.0 # may require sudo (Ubuntu) depending on configuration
cdk --version # must produce 2.1129.0
mkdir my-blueprints
cd my-blueprints
...
npm ERR! Found: aws-cdk-lib@2.290.0
npm ERR! node_modules/aws-cdk-lib
npm ERR!   aws-cdk-lib@"2.290.0" from the root project
npm ERR! 
npm ERR! Could not resolve dependency:
npm ERR! peer bundled aws-cdk-lib@"2.261.0" from @aws-quickstart/eks-blueprints@1.19.0
npm ERR! node_modules/@aws-quickstart/eks-blueprint
```

This message means that the version of CDK that the customer is using is different from the version of CDK used in EKS Blueprints. Locate the line `peer bundled` and check the expected version of the CDK. Make sure that in your `package.json` the version is set to the expected. In this example, `package.json` contained `"aws-cdk-lib": "2.290.0"`, while the expected version is `2.261.0`.
...
npm WARN peer bundled aws-cdk-lib@"2.261.0" from @aws-quickstart/eks-blueprints@1.19.0
```
